...
int main(void)
{
  ...
  uint32_t au32_initial_ticks = 0;
  uint32_t au32_end_ticks = 0;

  uint32_t value1 = 0;
  uint32_t value2 = 0;

  uint32_t realValue1 = 0;
  uint32_t realValue2 = 0;
  uint32_t realValue3 = 0;
  uint32_t realValue4 = 0;
  ...
  while (1)
  {
	...
	value1 = HAL_ADCEx_MultiModeGetValue(&hadc1);
	value2 =  HAL_ADCEx_MultiModeGetValue(&hadc3);

	realValue1 = value1 >> 16;
	realValue2 = value1 - (realValue1 << 16);

	realValue3 = value2 >> 16;
	realValue4 = value2 - (realValue2 << 16);

    IValue = realValue1 * filter - realValue2;
	QValue = realValue3 * filter - realValue4;
	...
	sprintf(endMessage, "After starting at %lu. It took %lu clock cycles to read one cycle\r\n", au32_initial_ticks, au32_end_ticks - au32_initial_ticks);
	...
	sprintf(endMessage, "It took %1.3f microseconds to read one cycle, corresponding to a sampling rate of %1.9lf MSPS\r\n", samplingTime, samplingRate);
