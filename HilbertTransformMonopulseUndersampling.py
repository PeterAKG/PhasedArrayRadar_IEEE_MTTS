...
def array_phase(n, th):    # spatial phase at element n
    return -2*np.pi*(n*d*np.sin(th))/lam
...
phi_spatial = array_phase(elem_idx, theta_true + (elem_idx/300))


fa = fc - np.round(fc/fs)*fs            # alias (−20 MHz for 60 MHz fs)
t  = np.arange(n_samples) / fs
v_rf = np.cos(2*np.pi*fa*t + phi_spatial[:,None])  # shape (N, n_samples)
...
plt.ylim(0,10); plt.grid(alpha=.3)
